...
def log_upload_save(sender, instance, created, **kwargs):
    if created:
        message = _(
            f"The file '{instance.title}' has been uploaded to the course '{instance.course_id}'."
        )
    else:
        message = _(
...
class UploadVideo(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "video_single", kwargs={"ref": self.course_id, "video_slug": self.slug}
        )
...
def log_uploadvideo_save(sender, instance, created, **kwargs):
    if created:
        message = _(
            f"The video '{instance.title}' has been uploaded to the course '{instance.course_id}'."
        )
    else:
        message = _(
            f"The video '{instance.title}' of the course '{instance.course_id}' has been updated."
        )
    ActivityLog.objects.create(message=message)
...
def log_uploadvideo_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(
        message=_(
            f"The video '{instance.title}' of the course '{instance.course_id}' has been deleted."
        )
    )
